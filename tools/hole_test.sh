#!/bin/bash
# تستِ طلاییِ «جای خالی»: کامپایل و اجرا در چند ثانیه، بی‌میکروفن و بی‌شبکه.
# خودِ pipe.m کامپایل می‌شود (همان کدی که در محصول می‌دود) و فقط `ZGoogleStream` بدل
...
set -euo pipefail
cd "$(dirname "$0")/.."
out="${TMPDIR:-/tmp}/zemzeme-hole-test"
work="${TMPDIR:-/tmp}/zemzeme-hole-work"
clang -fobjc-arc -O1 -Wall -Werror -I app/Sources-objc \
  app/Sources-objc/core.m app/Sources-objc/seg.m app/Sources-objc/pipe.m tools/hole_test.m \
