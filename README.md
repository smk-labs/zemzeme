...
- **پیش‌نمایش** (آزمایشی، اختیاری، پیش‌فرض خاموش، فقط حالت جمع در پنل): هر تکه‌ی
  رونویسی‌شده، حدود دو ثانیه بعد از بریده شدنش، **خاکستری** ته ادیتور پنل می‌نشیند؛
  سر پایان همه‌اش یک‌جا **سفید** می‌شود. رنگ یک معنی دارد و فقط همان: خاکستری یعنی
  هنوز تمام نشده، پس با پاس هوش مصنوعیِ روشن، خاکستری تا نشستنِ آن پاس می‌ماند.
  دکمه‌ی P روی نوار یا `Command راست + P`.

  **این استریم نیست و برگشتِ نسخه یک هم نیست.** هیچ سشن تازه‌ای باز نمی‌شود، هیچ
  بایت اضافه‌ای فرستاده نمی‌شود، و هیچ خطی از مسیر تشخیص عوض نمی‌شود: همان تکه‌های
  خط لوله‌اند، فقط زودتر دیده می‌شوند. متنِ پایانی مو به مو همان است که قبلا بود
  (روی ضبط ۰۷ سنجیده شد: ۱۲۹ واژه، ۳۳ گم‌شده، ۷۵٪، دقیقا عددِ جدولِ پایین). و چون
  هر تکه **قطعی** است و هیچ‌وقت پس گرفته نمی‌شود، درزی نیست که دوخته شود.

  **و پیشنهاد ما خاموش ماندنش است.** خواندنِ حرفِ خودت وسط حرف زدن رشته‌ی کلام را
  پاره می‌کند: چشم می‌رود سراغ تکه‌ای که چند ثانیه عقب‌تر است و زبان می‌ایستد. اگر
  آنچه لازم داری فقط «دارد می‌شنود؟» است، نشانِ ضربان همان را می‌گوید، بی هزینه.
