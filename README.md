...
چیزهایی که تازه‌کارها بیشتر از همه می‌پرسند:
...
- **متن را در پنل ویرایش کردم و باز حرف زدم؟** ویرایشت می‌ماند و حرفِ بعدی هم به
  آن اضافه می‌شود. هر چهار مقصد یک متن می‌گیرند: کرسر، کلیپ‌بورد، ردیف تاریخچه، و
  `text.txt`. تا ۲۸ اوت ۲۰۲۶ این‌طور نبود و در یک سشن ۱۰۵۷ نویسه سر کرسر رفت و ۲۵۹
  نویسه در کلیپ‌بورد. **بهایش یکی است:** ویرایش که کردی، پاس هوش مصنوعی تا آخرِ همان
  سشن خاموش می‌ماند. مدل کلِ متن را از نو می‌نویسد و متنِ تو را کسی از نو نمی‌نویسد.
- **صدا می‌رسد ولی هیچ متنی نمی‌آید؟** پنج تکه‌ی پشتِ سرِ هم که «حرفی نبود» بگیرند،
  پنل خودش می‌گوید: نزدیک‌تر حرف بزن، یا میکروفن را از تنظیمات سیستم › Sound عوض کن.
  پیش از این چنین سشنی تا سقفِ پنج دقیقه ساکت می‌ماند؛ ۱۸ اوت ۲۰۲۶ پنج دقیقه صدا رفت،
  ۴۷ تکه، هر ۴۷ تا بی‌حرف، و کاربر صفر نویسه و صفر خطا دید.
...
- **اینترنت وسط کار رفت؟** چیزی نمی‌بینی، و این عمدی است. حرف زدنت ادامه دارد، صدا
  ...
  همان لحظه تحویل می‌شود و بقیه بعدا سر جای خودشان می‌نشینند، حتی اگر پنل بسته شده
  باشد: در `text.txt` و `raw.txt` همان سشن و در همان ردیفِ تاریخچه، نه سر کرسر. پنل
  فقط یک شمار آرام نشان می‌دهد («یک تکه هنوز در راه است»)، نه پیام خطا و نه کاری که
  باید بکنی.
...
زمزمه خودش این را پیدا می‌کند و **روی پنل می‌گوید**، یک بار در هر اجرای اپ، با همین
راهنمایی. یک خط هشدار در منو هم هست که همان پنجره‌ی تنظیمات را باز می‌کند. پیش از این
فقط در منو و در `app.log` بود، و وسط دیکته کسی هیچ‌کدام را نگاه نمی‌کند. میان‌بر مک را
که عوض کنی، هشدار هم دیگر نمی‌آید. خودِ تیک را عمدا نمی‌زنیم، چون تنظیمِ سیستم مالِ
کاربر است. از تپ هم نمی‌شود بلعیدش: رویدادِ خامِ Command را که مصرف کنیم، هر `Cmd+C`
و `Cmd+V` هم با آن می‌رود.
...
- **دو روش نوشتن** («پیشرفته › روش نوشتن»): «درج مستقیم» متن را حرف‌به‌حرف می‌نویسد و
  ...
  در کلاینتِ ریموت همیشه راه کلیپ‌بورد گرفته می‌شود و این انتخاب رویش اثری ندارد: درج
  مستقیم آنجا فقط یک مشت `a` می‌نویسد. هر دو کلاینت این‌طورند، Windows App و
  `sdl-freerdp` روی لینوکس. دومی باندل آیدی ندارد و تا ۲۸ اوت ۲۰۲۶ از این قانون
  جا می‌ماند، پس حالا مقصد با نامِ فایلِ اجرایی هم شناخته می‌شود. در همان کلاینتِ لینوکس
  متن از پلِ SSH می‌رود نه از کانالِ RDP، و مهلتِ سینکِ کلیپ‌بورد کف ۱۲۰۰ میلی‌ثانیه
  دارد: اندازه‌گیریِ ۲۵ اوت ۲۰۲۶ رسیدنِ متن را ۵۲۳ تا ۶۹۹ میلی‌ثانیه نشان داد و با
  ۶۰۰ی پیش‌فرض گاهی متنِ قبلی پیست می‌شد.
...
- **نگه داشتن صدا** (پیش‌فرض روشن): ضبط همیشه انجام می‌شود، ولی ماندنش انتخاب توست.
  ...
  دیکته لازم شود. روشن یعنی هفت روز روی دیسک، بعد جاروی خودکار؛ خاموش یعنی سر پایان
  پاک شود. در هر دو حالت پاک شدن **پشتِ** نشستنِ متن روی دیسک است: صدا تنها توری است
  که تا امروز واقعا متنِ گم‌شده را برگردانده، پس متن که ننشیند، صدا هم نمی‌رود.
- **پاس هوش مصنوعی** (اختیاری، پیش‌فرض خاموش، **فقط روی متن**): فرمتینگ، و اصلاح
  ...
  خروجیِ کوتاه‌تر از ۷۰٪ ورودی رد می‌شود و متن خام سر جایش می‌ماند. و یک شرط دیگر:
  متن را که در پنل ویرایش کنی، این پاس تا آخرِ همان سشن اجرا نمی‌شود.
...
تست طلاییِ روشِ درج، و دو ادعا دارد که هر دو یک بار متن را خراب کردند. اول: **کلاینتِ
ریموت همیشه پیست می‌گیرد**، هر چه روشِ سراسری باشد و متن هر چه کوتاه باشد، و هر دو
کلاینت را می‌سنجد: آن که باندل آیدی دارد، و آن که ندارد و با نامِ فایلِ اجرایی شناخته
می‌شود. دلیلش در کیکد است، نه در سلیقه: رویدادِ تایپِ ما کیکد صفر است (همان A) با متنِ
یونیکد چسبیده به آن، و کلاینت ریموت روی `Keyboard Mode = Scancode` یونیکد را نمی‌خواند
و فقط اسکن‌کد را می‌فرستد، پس آن‌طرف یک مشت `a` می‌نشیند. دوم: **متنِ بلندی که نوشتنِ
