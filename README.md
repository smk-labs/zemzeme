...
![پنل زمزمه با متن دیکته‌شده](docs/img/panel-review.png)
...
**اگر فایل می‌خواهی:** `Zemzeme-2.3.0.dmg` در
...
![نوار پنل در حال گوش کردن](docs/img/panel-listening.png)
...
- **اینترنت وسط کار رفت؟** حرفی که نرسیده باشد پاک نمی‌شود: جایش در متن با
  `⟨جامانده⟩` علامت می‌خورد و همان لحظه هم به تو گفته می‌شود. دو بار پشت سر هم که
  نرسید، فرستادن می‌ایستد (ضبط ادامه دارد) و پنل می‌گوید. `Esc` را که بزنی فقط همان
  تکه‌ها دوباره می‌روند و متنشان سر جای خودشان می‌نشیند. تا پر نشده‌اند چیزی سر کرسر
  درج نمی‌شود، مگر خودت با دکمه‌ی درج (`I`) بگویی همین‌طور ببرش.
...
![کارت راهنمای میان‌برها](docs/img/cheatsheet.png)
...
- **جای خالی**: و اگر آن یک بار هم نگرفت، تکه **حذف نمی‌شود**. تکه‌ی بی‌صدا پیش از
  شبکه رد شده، پس هر تکه‌ای که به شبکه می‌رسد حرف دارد و متنِ خالی همیشه شکست است، نه
  جوابِ درست. جای آن تکه در متن با `⟨جامانده⟩` علامت می‌خورد، صدایش نگه داشته می‌شود،
  و همان لحظه به تو گفته می‌شود. دو شکستِ پشت سر هم یعنی اینترنت رفته: فرستادن
  می‌ایستد (ضبط نه) و پنل می‌گوید چه شد. `Esc` بعدی **فقط همان تکه‌ها** را دوباره
  می‌فرستد و متنشان سر جای همان نشانه می‌نشیند؛ نرسید که نرسید، متن در پنل می‌ماند و
  `Esc` بعدی یک تلاش تازه است. هیچ‌وقت متنِ سوراخ‌دار سر کرسر درج نمی‌شود، مگر خودت با
  دکمه‌ی درج (`I`) بگویی همین‌طور که هست ببرش.
...
- **سقف پنج دقیقه**: این ابزار دیکته است نه ضبط جلسه. سر پنج دقیقه سشن خودش تمام
  ...
  ![پنل رونویسی فایل با صف چند فایل](docs/img/batch-queue.png)
...
```bash
bash tools/hole_test.sh
```

همان خانواده‌ی بالا، یک پله بدتر: **حرفی که بی‌صدا پاک می‌شد.** تکه‌ای که رونویسی‌اش
خالی برمی‌گشت از فهرست متن می‌افتاد و بقیه به هم می‌چسبیدند، پس یک جمله‌ی گم‌شده هیچ
ردی نمی‌گذاشت. یک هفته اندازه گرفتیم: ۱۲ دقیقه از ۲۲۴ دقیقه دیکته (۱۳۹ تکه) این‌طور
رفت و ۳۴ سشن با سوراخِ دوخته‌شده درج شدند.

قاعده‌ای که تعمیرش را ساده کرد: **تکه‌ی بی‌صدا هیچ‌وقت به شبکه نمی‌رسد**، پس متنِ خالی
همیشه شکست است نه جوابِ درست. با همین یک جمله نه ناظر دسترسیِ شبکه لازم است، نه
backoff، نه حدس. تست خودِ `pipe.m` را کامپایل می‌کند و فقط سیمِ آخر به گوگل را بدل
می‌گذارد، پس مسیرِ واقعیِ تصمیم زیر تست است: نشانه سر جای خودش می‌نشیند، شمارنده بالا
می‌رود، دو شکستِ پشت سر هم **دقیقا یک بار** آژیر می‌زند، و تلاشِ دوباره متن را سر جای
نشانه‌ی **خودش** می‌گذارد نه سر جای اولی (وگرنه ترتیب حرف کاربر بی‌صدا جابه‌جا می‌شد).
سه ادعا هم می‌گویند چه چیزی **نباید** اضافه شود: هیچ `SCNetworkReachability` و هیچ
`NWPathMonitor`، چون پشت پروکسی، سیستم‌عامل «آنلاین» می‌گوید در حالی که گوگل نیست.
...
درخت کاری باید تمیز و با origin برابر باشد، هر هشت تست طلایی باید سبز باشند، و نسخه‌ی
...
**عکس‌های همین ریدمی** (`docs/img/`) دستی گرفته نشده‌اند؛ خودِ اپ درشان می‌آورد:

```bash
zemzeme --uishot docs/img
```
...
مکِ گرفته‌شده فرق می‌کنند. یک فایل صوتی هم که بدهی، به‌جای حالت‌های نمونه از یک اجرای
واقعی عکس می‌گیرد. `history.png` هم درمی‌آید و عمدا در ریپو نیست: تاریخچه متنِ واقعیِ
کاربر است.
