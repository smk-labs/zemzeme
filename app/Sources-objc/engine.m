...
@implementation ZEngine {
    ...
    BOOL _auditOpen;
    BOOL _stopping;
    BOOL _lateDropped;       // بلاکِ افتاده سرِ بستن، یک بار لاگ می‌شود نه هر بار
    BOOL _fileMode;
...
- (void)ingest:(NSData *)pcm {
    // دروازه حالا بعد از ایستادنِ میکروفن بسته می‌شود، ولی `removeTapOnBus` منتظرِ
    // بلاکِ در پرواز نمی‌ماند (audio.m)، پس یک بلاک هنوز می‌تواند دیر برسد. برداشتنِ
    // این شرط جوابش نیست: تا آن لحظه ضبط‌کننده `finish` خورده و طولش در هدر نشسته،
    // و یک `feed` تازه فایلِ بسته را دوباره باز می‌کند. پس می‌افتد، ولی دیگر بی‌صدا
    // نه. تا امروز هیچ عددی این افتادن را نمی‌دید و همان کوری بود که ~۰٫۲ ثانیه‌ی
    // B4 را به گردنِ خط لوله انداخت.
    if (_stopping) {
        if (!_lateDropped) {
            _lateDropped = YES;
            ZLog(@"engine: بلاکِ صدا سرِ بستن افتاد، %.0f میلی‌ثانیه، نه در audio.flac و نه در تکه‌ها",
                 pcm.length * 1000.0 / kZPcmBytesPerSec);
        }
        return;
    }
...
- (void)stop {
    ...
    // **اول منبع، بعد دروازه.** تا امروز `_stopping` بالای همین خط می‌نشست، و
    // `ingest` هر بلاکی را که بعدش از نخِ صدا می‌رسید دور می‌ریخت: نه به audio.flac
    // می‌رسید و نه به خط لوله. B4 را روی خط لوله دنبال می‌کردیم و آنجا هیچ‌وقت
    // نمی‌افتاد: خطِ «صدا بعد از بسته شدنِ خط لوله رسید» در شش سشنِ واقعی حتی یک بار
    // هم نخورد، چون بلاک یک لایه بالاتر مرده بود. حسابِ پوشش هم نمی‌دیدش، چون
    // مخرجش خودِ ضبط‌کننده است و بلاکی که به ضبط‌کننده نرسیده در هیچ کسری نمی‌آید.
    // در `cancel` عمدا برعکس است: آنجا دور ریختن خواسته‌ی کاربر است.
    [_mic stop];
    _mic = nil;
    _stopping = YES;
