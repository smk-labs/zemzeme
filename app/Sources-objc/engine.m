...
//
// سه کانال بیرون می‌رود و نه بیشتر: بلندی صدا (برای نشان)، **تکه‌های تمام‌شده** به
// ترتیب، و سر پایان یک متن. کانال دوم تازه است و باید دقیق فهمیده شود: آنچه از آن
// بیرون می‌رود متنِ در حال شکل‌گیری نیست، تکه‌ی **قطعیِ** رونویسی‌شده است، یعنی عینا
// همان چیزی که در متنِ پایانی هم می‌نشیند. پس هیچ‌وقت لازم نیست پس گرفته شود، و
// دفتر و راچت و پاک‌کنِ نسخه یک اینجا موضوعیت ندارند. موتور خودش هیچ تصمیمی با آن
// نمی‌گیرد و فقط ردش می‌کند؛ نشان دادن یا ندادنش کارِ رابط است.
#import "zemzeme.h"
...
- (void)buildPipes {
    ...
    // تنها کانالِ متنِ حین سشن، و عمدا تنها همین شکل: **تکه‌ی تمام‌شده، نه متنِ در
    // حال شکل‌گیری.** آنچه از اینجا بیرون می‌رود دقیقا همان چیزی است که سر پایان هم
    // در `_fa.text` می‌نشیند، پس نمایش دادنش هیچ‌وقت به «پس گرفتن» نمی‌کشد. اگر
    // دلیگیت این متد را نداشته باشد (مسیر اندازه‌گیری، مسیر دسته‌ای) هیچ اتفاقی
    // نمی‌افتد.
    //
    // فقط پاس فارسی: پاس دوم انگلیسی بیمه است نه متنِ کاربر، و نشان دادنش یعنی دو
    // رونویسیِ همان صدا هم‌زمان روی صفحه بروند.
    __weak typeof(self) weak = self;
    _fa.onPart = ^(NSString *t) {
        dispatch_async(dispatch_get_main_queue(), ^{
            typeof(self) me = weak;
            // سر پایان، تکه‌های ته‌مانده همین‌جا می‌رسند؛ آن لحظه پنل دارد «یک لحظه…»
            // می‌گوید و چند ثانیه بعد متن کامل می‌نشیند. یک پرشِ خاکستری در این فاصله
            // فقط نویز است.
            if (!me || me->_stopping) return;
            id<ZEngineDelegate> d = me->_delegate;
            if ([d respondsToSelector:@selector(enginePart:)]) [d enginePart:t];
        });
    };
