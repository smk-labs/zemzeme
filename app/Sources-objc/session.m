...
- (void)queueChanged {
    // یک **بند**، نه یک تحویل‌گیرنده. تکه‌ی دیررس اصلا از این در نمی‌آید: سشنِ بسته
    // را `onFinish` همان لحظه آزاد می‌کند و بلاکِ `__weak` روی نال خالی می‌شود. تا
    // امروز اینجا شاخه‌ای بود که ادعا می‌کرد آن تکه را می‌گیرد و نمی‌گرفت، و همان
    // ادعا باگ B1 را قایم نگه داشت (شاهدش: «تکه‌ی دیررس نشست» در کلِ تاریخِ app.log
    // صفر بار). حالا `endNow` صف را به `ZAdoptOrphanQueue` می‌سپارد و متن از آنجا
...
// صف اینجا کشته نمی‌شود، ولی **صاحبش عوض می‌شود**. تا امروز همین‌جا نوشته بود
// «تکه‌ی در راه بعد از رفتنِ پنل هم می‌رسد و جای خودش می‌نشیند»، و راست نبود: سشن
// چند خط پایین‌تر آزاد می‌شود و کال‌بکِ صف روی نال خالی می‌ماند (باگ B1).
- (void)endNow {
    if (_finished) return;
    _finished = YES;
    ...
    // و بعد از آن، صف به صاحبِ تازه سپرده می‌شود. **بعد**، چون `applyAudioPolicy`
    // ضبط را می‌بندد و تکه‌ی در راه صدایش را از همان فایل می‌خواند.
    ZAdoptOrphanQueue(_queue, _sessionDir.lastPathComponent, _rewrite, _rewritten);
    [self stopClock];
    [self unwirePanel];
    [_dot hide];
    [_panel hide];
