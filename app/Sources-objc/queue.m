...
// متنی روی دیسک هست یا نه. شرطِ پاک کردنِ صدا، و همان قاعده‌ی B2 یک پله بالاتر:
// **تور آخر از همه می‌رود.** پایینِ همین فایل مسیرِ صفِ برداشته‌شده را می‌گیرد، ولی
// مسیرِ زنده (`applyAudioPolicy` در session.m) از این در رد می‌شد و صدای سشنی را که
// هیچ متنی نداده بود پاک می‌کرد. و «هیچ متنی نداد» دقیقا همان حالتی است که صدا تنها
// نسخه‌ی حرفِ کاربر است: میکروفنِ کرِ ۱۸ اوت ۲۰۲۶، ۴۷ تکه و صفر نویسه، با تاگلِ خاموش
// تنها شاهدش را هم از دست می‌داد.
BOOL ZTextLanded(NSURL *sessionDir) {
    NSURL *txt = [sessionDir URLByAppendingPathComponent:@"text.txt"];
    if ([NSFileManager.defaultManager attributesOfItemAtPath:txt.path error:nil].fileSize) return YES;
    ZLog(@"session: متنی روی دیسک ننشست، پس صدا هم دست نمی‌خورد");
    return NO;
}
