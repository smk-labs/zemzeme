...
// متنِ معلقِ سشنِ در حال تخلیه. فقط carry را درازتر می‌کند، هیچ‌وقت کوتاه‌تر: آن
// استریم گاهی **بعد از** چرخش تازه واژه‌ی سخت را می‌فهمد و بی این، آن کلمه هیچ‌جا نبود.
- (void)drainInterim:(NSString *)text;
...
// چند ثانیه بی‌نتیجه به حساب **این** استریم نوشته می‌شود. حساب خالص روی سه زمان، پس
// تست بی‌میکروفن و بی‌شبکه می‌دواندش.
//
// کلِ نکته در همین max است: استریمی که با بازپخش باز می‌شود اول باید همان صدای کهنه
// را بالا بفرستد و بشنود، و در آن فاصله نتیجه‌ای نمی‌آید. مهلت (`graceUntil`) برای
// همین است، ولی باید از **ساعت** کم شود نه فقط از سنجش. قبلا فقط سنجش را معلق
// می‌کرد و ساعت از لحظه‌ی باز شدن می‌رفت، پس با ۶ ثانیه بازپخش، همان لحظه‌ای که مهلت
// تمام می‌شد شمارنده از قبل ۸ بود، یعنی بالای سقفِ ۵: استریم در اولین tickِ بعد از
// مهلت کشته می‌شد، بی‌آنکه یک ثانیه فرصتِ جواب دادن داشته باشد.
//
// و حلقه می‌شد: هر کشتن بازپخشِ درازتری به بعدی می‌داد، مهلتِ بعدی درازتر می‌شد و
// کشتنِ بعدی هم همان‌قدر حتمی، تا بازپخش روی سقفِ ۱۲ ثانیه میخ شود. سشن ۰۹:۲۲:۱۵
// همین دستگاه عینا: «stalled 6s» بعد «stalled 10s» و بعد میخ؛ ۲۷ ثانیه حرف زدن ۸۱
// نویسه داد. کاربر این را «قفل کردن» می‌بیند، و چون گوگل سرِ اصطلاحِ انگلیسیِ وسط
// فارسی چند ثانیه لال می‌شود، همیشه از همان‌جا شروع می‌شد.
static inline NSTimeInterval ZStallSeconds(NSDate *lastResultAt, NSDate *graceUntil, NSDate *now) {
    if (!lastResultAt || !now) return 0;
    NSDate *since = lastResultAt;
    if (graceUntil && [graceUntil compare:since] == NSOrderedDescending) since = graceUntil;
    return [now timeIntervalSinceDate:since];
}
...
// ---------- نوبتِ پاس ----------
// انتقالِ جمینای دو مصرف‌کننده دارد (پاس نهایی و بهبود پرامپت) و بیشتر از یک کارِ
// همزمان را تحمل نمی‌کند. تا امروز نگهبانش پرچمِ خودِ پنل‌ها بود، و پرچمِ پنل‌به‌پنل با
// هم جمع نمی‌شود: پاس نهاییِ سشن و کارِ پنل فایل واقعا روی هم می‌افتادند، تسکِ در پرواز
// روی هم نوشته می‌شد و یک `resetCancel` لغوِ دیگری را پاک می‌کرد. حالا نگهبان خودِ
// انتقال است. دلیل‌ها و شکلِ کار در `passlock.m`.
//
// دو چیز که از بیرون باید دانست:
//   · نوبت مالِ **کل پاس** است نه یک تماس؛ وگرنه بین دو تماسِ پاس نهایی کارِ دیگری
//     جا می‌شد.
//   · آزاد شدن کارِ `dealloc` است. نوبت را در متغیر محلی نگه دار و در هیچ بلاکی نگیر:
//     هر `return` خودش آزاد می‌کند، و هیچ نقطه‌ی بازگشتی نمی‌تواند فراموشش کند.
@class ZPassLock;

@interface ZPassLease : NSObject
@property (nonatomic, readonly, copy) NSString *owner;
@property (nonatomic, readonly) BOOL cancelled;    // بین دو مرحله پرسیده می‌شود، نه فقط اولش
@end

extern NSString *const ZPassOwnerFinal;      // «پاس نهایی»
extern NSString *const ZPassOwnerEnhance;    // «بهبود پرامپت»

@interface ZPassLock : NSObject
// نال یعنی مشغول، و همان لحظه: نه بلوکه، نه صف. `busy` یک خط فارسی می‌گیرد که اسمِ
// کارِ در جریان را دارد و مستقیم روی خط وضعیت می‌نشیند.
- (ZPassLease *)claim:(NSString *)owner busy:(NSString **)busy;
- (BOOL)cancelOwner:(NSString *)owner;    // NO یعنی کارِ در جریان مالِ این صاحب نبود
- (BOOL)armTask:(NSURLSessionTask *)task session:(NSURLSession *)session;    // NO یعنی لغو شده
- (void)disarm;
- (NSString *)busyOwner;    // نال یعنی آزاد؛ برای لاگ و تست
@end

// ---------- پاس نهایی: صدا، در یک تماس، تا متن تمیز ----------
...
//
// یک تماس، نه دو. قبلا دو تا بود (رونویسی مو‌به‌مو، بعد پاس متنی روی همان) چون یک
// تماسِ اولیه متنِ درهم و دوباره‌نویسی‌شده داده بود (۲۰۷۲ کلمه به جای ۹۶۵). ولی
// هزینه‌اش را اندازه گرفتیم و صاف روی صورت کاربر بود: دو رفت‌وبرگشت، و متنِ کامل دو
// بار تولید می‌شد (یک بار مو‌به‌مو، یک بار تمیز)، یعنی ۱۵ تا ۱۸ ثانیه و دو برابر
// توکنِ خروجی. تصمیم: یک تماس، و آن شکستِ قدیمی با خودِ پرامپت مهار می‌شود نه با یک
// مرحله‌ی اضافه (`prompts/transcribe.md`: سقفِ خروجی خودِ گفتار است، جمله دو بار
// نوشته نمی‌شود). سرعت اینجا فیچر است، نه راحتی.
//
...
@property (nonatomic, copy) NSString *text;        // متن نهایی
...
// یک پاس در هر لحظه، و نگهبانش خودِ انتقال است نه فراخوان (`ZPassLock`). پاسِ دوم که
// همزمان شروع شود بلوکه نمی‌شود: همان لحظه با `error` فارسی برمی‌گردد و متن قبلی
// دست‌نخورده می‌ماند.
...
// می‌زند، پس این متدها همان انتقال را قرض می‌دهند و لایه‌ی دومی ساخته نمی‌شود.
// نوبت هم از همین‌جا گرفته می‌شود: قفل یکی است و هر دو مصرف‌کننده از همان می‌گیرند،
// وگرنه «یک کار در هر لحظه» فقط داخل هر کدام برقرار می‌شد نه بینشان.
//
...
// نوبتِ کل پاس. نال یعنی مشغول، و `busy` همان یک خط فارسی است که به کاربر نشان
// می‌دهیم. آزاد کردن کارِ فراخوان نیست: نوبت را در متغیر محلی نگه دار و تمام.
- (ZPassLease *)claimPass:(NSString *)owner busy:(NSString **)busy;
// لغو به نامِ صاحب، نه لغوِ هرچه در پرواز است. `-cancel` بالا همین است با
// `ZPassOwnerFinal`؛ `ZEnhance` همین را با `ZPassOwnerEnhance` می‌زند.
...
// نوبت را از همان `ZPassLock` می‌گیرد که پاس نهایی می‌گیرد، پس اگر پاس نهایی در جریان
// باشد این همان لحظه با `error` فارسی برمی‌گردد و متن قبلی سر جایش می‌ماند.
...
// و دیگر نه.
//
// `pasteIfRefused` یعنی «این متن یکجاست و تکرارشدنی نیست». اپی که نوشتنِ اتمیک را رد
// کرد، همان اپی است که رویدادِ ساختگی را هم کامل نمی‌گیرد؛ پس متنِ بلند از پیست
// می‌رود، نه از رگبار. دُمِ زنده‌ی حالت کرسر NO می‌دهد: تکه‌هایش کوچک‌اند، دفتر
// حسابشان را دارد، و هر پیست کلیپ‌بورد را می‌خورد.
