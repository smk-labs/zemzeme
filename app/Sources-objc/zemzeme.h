...
// ---------- عکسِ درون‌پروسه‌ای ----------
// یک ویو را به PNG می‌دهد، **همیشه دو برابر**، و «همیشه» یعنی مستقل از صفحه‌نمایش.
//
// `bitmapImageRepForCachingDisplayInRect:` ضریب را از صفحه‌ای می‌گیرد که پنجره
// رویش نشسته. روی مکِ رتینا ۲x درمی‌آمد و روی همان مک با یک مانیتور خارجیِ معمولی
// ۱x، پس عکس‌های ریدمی با هر بار گرفتن نصف یا دو برابر می‌شدند و آن جمله‌ی ریدمی
// («نه عکس‌ها با هر مکِ گرفته‌شده فرق می‌کنند») از اول درست نبود. اینجا تصمیم از
// دستِ صفحه گرفته می‌شود.
...
// zemzeme --keyacl: آیا همین باینری کلید را بی‌پنجره می‌خواند؟ ۰ بله، ۱ کلید هست
// ولی اجازه نه، ۲ کلیدی نیست. build.sh از این می‌فهمد بیلد تازه اجازه‌ی کی‌چین
// می‌خواهد یا نه (فهرست Partition به cdhash بسته است و هر بیلد cdhash تازه دارد).
