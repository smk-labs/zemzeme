...
// خطِ عربی/فارسی یا لاتین؟ فقط برای تصمیمِ زیر لازم است، پس همین حد کافی است.
static BOOL ZSeamIsLatin(NSString *t) {
    NSUInteger lat = 0, ara = 0;
    for (NSUInteger i = 0; i < t.length; i++) {
        unichar c = [t characterAtIndex:i];
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) lat++;
...
static double ZSeamTokenSim(NSString *x, NSString *y) {
    ...
    if (!n || !m) return 0.0;
    // **دو خطِ مختلف یعنی نمی‌دانیم، نه یعنی بی‌ربط.** گوگل یک واژه‌ی انگلیسی را در
    // دو استریم دو جور می‌نویسد: «Docker» و «داکر». فاصله‌ی لِوِنشتاینِ آن دو تقریبا
    // بیشینه است، پس یک واژه کافی بود تا هم‌ترازیِ کلِ پنجره بریزد و درز، متنی را که
    // واقعا تکراری بود دوباره بنویسد. حرف‌به‌حرف مقایسه کردنِ دو خطِ مختلف بی‌معنی
    // است، پس به‌جای «صفرِ قاطع» یک «نصفِ مردد» برمی‌گردد: یک واژه‌ی دوزبانه دیگر
    // پنجره را نمی‌ریزد، و در عوض هم‌ترازی را هم به تنهایی نمی‌سازد.
    if (ZSeamIsLatin(x) != ZSeamIsLatin(y)) return 0.5;
...
NSString *ZUncoveredTail(NSString *whole, NSString *covered) {
    ...
    //
    // ...ولی «امتیاز پایین» و «بی‌ربط» یکی نیستند، و فرقشان دقیقا سرِ حرفِ دوزبانه
    // خودش را نشان می‌دهد. گوگل یک واژه‌ی انگلیسی را در دو استریم دو جور می‌نویسد،
    // یک بار «Docker» و یک بار «داکر». برای این نرمال‌ساز آن دو هیچ شباهتی ندارند،
    // پس یک توکن کافی است تا هم‌ترازیِ کلِ پنجره بریزد و امتیاز به صفر برسد، در
    // حالی که همان جمله است. آن‌وقت این خط کلِ دُم را دور می‌ریخت: جمله‌ی کامل، به
    // خاطر یک کلمه‌ی انگلیسی.
    // پس یک سنجه‌ی دوم: چند درصدِ توکن‌های covered اصلا شریکی در whole پیدا کرده‌اند.
    // متنِ واقعا بیگانه اینجا هم نزدیک صفر می‌گیرد (تست طلایی سر جایش می‌ماند)، ولی
    // همان جمله با یک واژه‌ی دوزبانه بالای نصف می‌ماند و نجات پیدا می‌کند.
    if (score < 0.60) {
        NSUInteger paired = 0;
        for (NSUInteger j = 0; j < L; j++) {
            for (NSUInteger i = 0; i < P; i++) {
                if (ZSeamTokenSim(nA[i], nB[j]) >= kZSeamStrongSim) { paired++; break; }
            }
        }
        // یک‌سوم، نه نصف: یک واژه‌ی دوزبانه می‌تواند هم‌ترازی را چنان بریزد که فقط
        // چهار از نُه واژه شریک پیدا کنند، و آن هنوز همان جمله است. متنِ واقعا
        // بیگانه (تست طلایی: نام روزها در برابر نام آب‌وهوا) صفر می‌گیرد، پس این
        // مرز هنوز جدایشان می‌کند.
        // اینجا لاگ نمی‌زنیم: این فایل عمدا هیچ وابستگی‌ای ندارد تا تست طلایی‌اش
        // بتواند تنهایی کامپایلش کند.
        if (paired * 3 < L) return @"";
    }
    if (bestI >= n) return @"";
