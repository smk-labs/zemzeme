...
- (NSArray<NSDictionary *> *)sections {
    return @[
        ...
        // این سه به سشن ربطی ندارند، پس بخش جدا دارند: تازه‌کاری که هنوز دیکته‌ای شروع
...
- (NSArray<NSArray *> *)notes {
    ...
    [n addObject:@[@"info.circle", @"جز H و F و A، بقیه‌ی حرف‌ها فقط وسط دیکته کار می‌کنند", @NO]];
    // پاس هوش مصنوعی روی **متن** اجرا می‌شود، نه روی صدا، پس تنها شرطش کلید است.
    // در نسخه یک شرط دومی هم داشت (صدایی روی دیسک باشد) و همان دو شرطی بودن باعث
    // می‌شد کاربر نفهمد چرا دکمه کار نمی‌کند.
    if (ZSettings.shared.finalPassEnabled && !ZFinalPass.hasKey) {
        [n addObject:@[@"exclamationmark.triangle",
                       @"تمیز کردن متن روشن است ولی کلید Gemini نیست؛ از منو «کلید Gemini…» را بزن",
                       @YES]];
    }
