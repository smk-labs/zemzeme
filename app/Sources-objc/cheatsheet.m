...
- (NSArray<NSDictionary *> *)sections {
    return @[
        @{@"title": @"زدن Command راست",
          @"rows": @[
              @[@[@"⌘", @"⌘"], @"mic.fill",     @"شروع و پایان دیکته"],
              @[@[@"⌘"],       @"pause.circle", @"حرفت که تمام شد، یک بار بزن: متن می‌آید"],
          ]},
        @{@"title": @"Command راست + یک حرف، وسط دیکته",
          @"rows": @[
              @[@[@"C"],     @"doc.on.doc",        @"کپی متن تا اینجا"],
              @[@[@"I"],     @"text.insert",       @"درج متن سر کرسر"],
              @[@[@"D"],     @"trash",             @"دور ریختن متن درج‌نشده و صدای ضبط‌شده"],
              @[@[@"L"],     @"globe",             @"عوض کردن زبان: فارسی و انگلیسی"],
              @[@[@"E"],     @"square.and.pencil", @"جابه‌جایی بین دو حالت: جمع در پنل، کنار کرسر"],
              @[@[@"S"],     @"ear.badge.waveform", @"حساسیت بالای میکروفن: برای پچ‌پچ و اتاق ساکت"],
              @[@[@"Space"], @"pause.circle",      @"مکث و ادامه"],
          ]},
        // این‌ها به سشن ربطی ندارند، پس بخش جدا دارند: تازه‌کاری که هنوز دیکته‌ای شروع
        // نکرده هم باید بتواند راهنما را باز کند و فایل رونویسی کند.
        // ترتیب همان ترتیب نوار پنل و منو است، تا هر سه سطح یک نقشه بدهند: اول ابزارها
        // (F و T و H)، بعد تاگل‌ها (B و A).
        @{@"title": @"Command راست + این‌ها، هر وقت",
          @"rows": @[
              @[@[@"F"], @"arrow.up.doc",        @"پنل رونویسی فایل صوتی"],
              @[@[@"T"], @"clock.arrow.circlepath", @"تاریخچه: بیست متن آخر، با درج و کپی"],
              @[@[@"H"], @"questionmark.circle", @"همین کارت راهنما"],
              @[@[@"B"], @"character.book.closed", @"شنیدن دوزبانه: همین صدا را انگلیسی هم بشنود"],
              @[@[@"P"], @"captions.bubble",     @"پیش‌نمایش خام، هم‌گام با حرف زدن: روشن و خاموش"],
              @[@[@"A"], @"sparkles",            @"تمیز کردن متن با هوش مصنوعی: روشن و خاموش"],
              @[@[@"G"], @"signature",           @"امضا: یک خط کوتاه ته هر متن. متنش را از منوبار عوض کن"],
          ]},
        @{@"title": @"بدون کلید کمکی",
          @"rows": @[
              @[@[@"Esc"], @"stop.circle", @"این کارت را می‌بندد، وگرنه پایان دیکته و درج متن"],
          ]},
        @{@"title": @"بدون کیبورد",
          @"rows": @[
...
- (NSArray<NSArray *> *)notes {
    ...
    // اولین پانویس، و وضعیتِ همین لحظه را می‌گوید نه قاعده‌ی کلی را: با پیش‌نمایشِ
    // خاموش (پیش‌فرض) کاربر باید بداند سکوتِ صفحه عمدی است، وگرنه فکر می‌کند اپ گیر
    // کرده؛ و با پیش‌نمایشِ روشن باید بداند خاکستری یعنی هنوز تمام نشده، وگرنه متنِ
    // نیمه‌کاره را نتیجه‌ی نهایی می‌بیند و ناامید می‌شود.
    [n addObject:ZSettings.shared.previewStream
        ? @[@"captions.bubble",
            @"پیش‌نمایش روشن است: خاکستری خام است و غلط دارد؛ متن درست سر پایان می‌آید.", @NO]
        : @[@"info.circle",
            @"در حین حرف زدن هیچ متنی نشان داده نمی‌شود. تا نشان سبز است یعنی می‌شنوم.", @NO]];
    ...
    // پلِ میان این کارت و نوار پنل. بی این یک جمله، نوار پانزده آیکونِ بی‌اسم است و
    // حرف‌های ریزِ زیرشان به چشم تزئین می‌آیند؛ با آن، همین کارت تبدیل می‌شود به
    // فهرستِ خودِ نوار. تولتیپ نمی‌توانست این کار را بکند: پنل نان‌اکتیویتینگ است و
    // مک هیچ‌وقت رویش تولتیپ نمی‌کشد.
    [n addObject:@[@"keyboard", @"همین حرف‌ها زیر دکمه‌های نوار پنل هم نوشته‌اند: هر دکمه، میان‌بر خودش", @NO]];
    [n addObject:@[@"info.circle", @"جز F و H و T و B و P و A و G، بقیه‌ی حرف‌ها فقط وسط دیکته کار می‌کنند", @NO]];
    // هشدارِ «روشن است ولی کلید ندارد» برداشته شد چون آن حالت دیگر ساخته نمی‌شود:
    // روشن کردنش یا کلید می‌گیرد یا خودش خاموش می‌شود، و لانچ هم همین را می‌سنجد.
    ...
    if (!ZSettings.shared.internalHotkey) {
        [n addObject:@[@"exclamationmark.triangle",
                       @"دو بار زدن Command راست کار نمی‌کند: در «پیشرفته»، «میان‌بر داخلی زمزمه» را روشن کن", @YES]];
    }
    return n;
}
