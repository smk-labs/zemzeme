...
- (void)makeShots:(NSString *)dir {
    ...
    for (NSArray *pair in states) {
        ...
        [self clearEditor];
        if (m.review) {
            // متنِ نمونه یک **دیکته‌ی واقعی** را نشان می‌دهد، نه توضیحِ خودِ پنل را. دو
            // دلیل: عکس باید همان چیزی باشد که کاربر می‌بیند، و متنِ قبلی ادعای
            // «با کیبورد خودت قابل ویرایش است» را می‌کرد، در حالی که پنل nonactivating
            // است و کیبورد به ادیتور نمی‌رسد. عکسی که چیزی را وعده بدهد که نیست،
            // بدتر از عکس نداشتن است.
