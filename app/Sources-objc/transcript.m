...
- (void)addFinal:(NSString *)text weld:(BOOL)weld {
    ...
    // ...ولی نه وقتی این متن قرار است جوش بخورد. جوش یعنی این اولین متنِ استریمی است
    // که با صدای هم‌پوشان باز شده، پس سرِ دُمِ خاکستری‌اش تشخیصِ **دوباره**ی صدایی است
    // که استریمِ قبلی قطعی‌اش کرده، نه حرفی که جا مانده. فیکسچر ۹ عینا: استریمِ تازه
    // از ناحیه‌ی هم‌پوشانی یک «خب» شنید که گوگل خودش در متنِ قطعی انداخته بود، و
    // نجاتِ سر همان را وسطِ جمله دوباره می‌نشاند. دو سازوکار سرِ یک ناحیه با هم
    // می‌جنگیدند: جوش تکرارِ هم‌پوشانی را برمی‌داشت، نجاتِ سر برش می‌گرداند.
    if (t.length) {
        NSString *head = weld ? @"" : ZUncoveredHead(_interim, t);
        if (head.length) {
...
// متنِ معلقِ سشنِ در حال تخلیه، وقتی هنوز قطعی نشده. تا امروز دور ریخته می‌شد و
// دقیقا همان‌ها بودند که سرِ واژه‌ی سخت تازه از راه می‌رسیدند: استریمِ قدیمی چند صد
// میلی‌ثانیه **بعد از** چرخش بالاخره آن کلمه را می‌فهمید، ولی چون فقط متنِ قطعی‌اش
// گوش شنوا داشت، اگر قطعی نمی‌رسید (مهلتِ ۸ ثانیه، یا مرگِ اتصال) آن کلمه هیچ‌جا
// نبود: نه در carry که لحظه‌ی چرخش عکس گرفته شده بود، نه در استریمِ تازه که آن صدا
// را نشنیده بود.
//
// راچت یک‌طرفه است، پس این فقط می‌تواند carry را **درازتر** کند. نه چیزی پس گرفته
// می‌شود، نه جای متنِ قطعی تنگ می‌شود: هرچه drainFinal بعدا بپوشاند همان‌جا مصرف
// می‌شود، و هرچه نپوشاند سرِ endDrain قطعی می‌شود. همان قاعده‌ی همیشگی.
- (void)drainInterim:(NSString *)text {
    if (!_draining) return;
    NSString *t = [(text ?: @"") stringByTrimmingCharactersInSet:
                   NSCharacterSet.whitespaceAndNewlineCharacterSet];
    if (!t.length) return;
    NSString *grown = ZInterimRatchet(_carry, t);
    if ([grown isEqualToString:_carry]) return;
    ZEventLogWrite(@{@"k": @"draininterim", @"text": t});
    _carry = [grown copy];
    // ‏ZUncoveredTail با پوششِ خالی خودِ whole را برمی‌گرداند، پس اول تخلیه هم درست است.
