...
@implementation ZSession {
    ...
    // ---------- صدا و پاس نهایی ----------
    // **این کامنت قبلا می‌گفت صدای هر سشن ضبط می‌شود. غلط بود.** `recorderForMode:`
    // فقط در حالت یادداشت، یا وقتی تنظیمِ «ضبط سشن‌ها» روشن باشد، ضبط‌کننده می‌دهد؛
    // در بقیه‌ی حالت‌ها nil برمی‌گرداند و صدا اصلا روی دیسک نمی‌رود. در ۱۳۸ سشن از
    // ۱۴۳ سشنِ لاگ، خط پایانی `audio 0s` است و دلیلش همین است، نه خرابی.
    // پیامدش را باید دانست: «پاس نهایی» صدا می‌خواهد، پس تا آن تنظیم روشن نشود روی
    // سشن‌های معمولی در دسترس نیست. فایل تنبل ساخته می‌شود، پس سشن بی‌صدا چیزی روی
    // دیسک نمی‌گذارد.
    ZRecorder *_recorder;
...
// **سشن می‌توانست وسط متدِ خودش بمیرد.** تنها ارجاع قویِ این شیء `_session` در دلیگیت
// اپ است، و مسیر پایان همان ارجاع را از داخلِ خودش صفر می‌کند: `teardown` بلاک
// `onFinish` را صدا می‌زند، دلیگیت `_session = nil` می‌گذارد، شمارنده صفر می‌شود و
// همان‌جا `dealloc` اجرا می‌شود. هر خطی که بعدش بیاید روی حافظه‌ی آزادشده می‌نشیند.
//
// کرش واقعی و تکرارشونده بود، نه احتمالِ روی کاغذ: شش گزارش در دو روز، همه با یک
// امضا (`objc_msgSend` داخل `finishNow`، گیرنده‌ای که دیگر نبود) و از شش فراخوانِ
// مختلف: URL، منو، Esc، و مسیر خروج اپ. چون UAF است، فقط وقتی می‌ترکد که حافظه‌ی
// آزادشده در همان لحظه دست دیگری بیفتد، پس گاهی سالم رد می‌شد و گاهی نه. برای کاربر
// این یعنی زمزمه سرِ تمام کردن دیکته بی‌صدا ناپدید می‌شود.
//
// این ماکرو self را تا آخر تابع زنده نگه می‌دارد. `objc_precise_lifetime` لازم است،
// وگرنه ARC اجازه دارد آزادسازی را زودتر از پایان اسکوپ انجام دهد و همان حفره باز
// می‌ماند. روی هر چهار درِ پایان می‌نشیند، چون هر چهار در به `onFinish` می‌رسند.
#define ZHoldSelfAlive() \
    __attribute__((objc_precise_lifetime)) __unused ZSession *zAlive_ = self
...
- (void)teardown {
    ...
    ZEventLogStop();
    // خبر دادن **آخرین** کار است، و بلاک پیش از صدا زدنش برداشته می‌شود. قبلا برعکس
    // بود (`onFinish()` بعد `self.onFinish = nil`) و آن یک خط تضمینی روی شیءِ مرده
    // می‌نشست: دلیگیت داخل همان بلاک آخرین ارجاع را صفر کرده بود. برداشتنِ زودهنگام
    // همان کارِ «دو بار خبر ندهیم» را هم می‌کند، چون بلاک دیگر سر جایش نیست.
    void (^done)(void) = self.onFinish;
    self.onFinish = nil;
    if (done) done();
}
