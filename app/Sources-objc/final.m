...
// ---------- zemzeme --keyacl ----------
// «آیا **همین باینری** می‌تواند کلید را بی‌پنجره بخواند؟» و بس: نه شبکه، نه متغیر
// محیطی، نه فال‌بکِ ابزار `security`. آن دو راهِ دیگر جوابِ سوال را عوض می‌کنند، چون
// از کنارِ همان چیزی رد می‌شوند که داریم اندازه می‌گیریم.
//
// چرا اصلا هست: کی‌چینِ فایلی هر آیتم را به **cdhash** پروسه‌ی سازنده می‌بندد
// (فهرست Partition). امضای زمزمه با گواهی خودی است و Team ID ندارد، پس مک چیزی
// جز cdhash ندارد که بنویسد؛ و cdhash با هر بیلد عوض می‌شود. یعنی هر بیلدِ تازه از
// نظر کی‌چین یک اپِ دیگر است و «همیشه اجازه بده»‌ی دیروز به آن نمی‌رسد. build.sh با
// همین کد می‌فهمد بیلدِ تازه اجازه دارد یا نه، و اگر نه، همان‌جا اجازه را تازه می‌کند
// تا پنجره‌ی رمز وسطِ دیکته بالا نیاید.
//
// خروجی: ۰ یعنی می‌خواند، ۱ یعنی کلید هست ولی این بیلد اجازه ندارد، ۲ یعنی کلیدی
// ذخیره نشده (کاری هم لازم نیست).
int ZKeyACLMain(void) {
    BOOL blocked = NO;
    NSString *k = ZKeyFromKeychain(NO, &blocked);
    if (k.length) { printf("کلید خوانده شد، این بیلد اجازه دارد\n"); return 0; }
    if (blocked) { fprintf(stderr, "کلید هست ولی این بیلد اجازه‌ی خواندنش را ندارد\n"); return 1; }
    fprintf(stderr, "کلیدی در Keychain ذخیره نشده\n");
    return 2;
}
