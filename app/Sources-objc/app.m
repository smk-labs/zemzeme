...
- (void)showFirstRunWelcomeIfNeeded {
    ...
    // متن ساده و بس: NSAlert مارک‌داون رندر نمی‌کند و ستاره‌ها را عینا نشان می‌دهد.
    // یک بار همین‌جا **پررنگ** نوشته شده بود و اولین چیزی که هر کاربر تازه می‌دید،
    // دو ستاره‌ی سرگردان وسط جمله بود. تاکید با جای جمله ساخته می‌شود، نه با نشانه.
    //
    // ترتیب هم اتفاقی نیست و همان ترتیبِ اولین دقیقه‌ی کاربر است: چطور شروع کنم، در
    // حین حرف زدن چه می‌بینم (هیچ، و این عجیب‌ترین تصمیمِ نسخه دو است)، چطور تمام
    // کنم، متن کجا می‌رود، و آن نوارِ پر از آیکون چیست.
    a.informativeText =
        @"شروع: Command راست را دو بار پشت هم بزن و حرف بزن.\n"
         "تا وقتی حرف می‌زنی هیچ متنی نشان داده نمی‌شود، و این عمدی است؛ نقطه‌ی سبز "
         "یعنی می‌شنوم.\n"
         "پایان: یک بار Command راست. متن یکجا در پنل می‌نشیند.\n"
         "Esc: همان متن سر کرسر درج می‌شود و سشن بسته می‌شود.\n\n"
         "زیر هر دکمه‌ی نوار پنل یک حرف ریز نوشته شده، و همان حرف با Command راست "
         "همان کار را می‌کند. فهرست کاملشان: Command راست + H.\n\n"
         "الان مک اجازه‌ی Accessibility می‌خواهد؛ بده، وگرنه زمزمه نمی‌تواند متن را "
         "جای کرسر بنویسد. میکروفن سرِ اولین دیکته پرسیده می‌شود.\n\n"
         "رایگان است: نه حساب کاربری، نه کلید، نه سقف. در عوض صدا به سرور گوگل "
         "می‌رود و پشت این سرویسِ رایگان قراردادی نیست، پس فرض کن گوگل می‌تواند از "
         "آن برای بهتر کردن مدل‌هایش استفاده کند. (تنها چیزی که کلید می‌خواهد «تمیز "
         "کردن متن» است، و خودش هم اختیاری.)";
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
    ...
    prev.toolTip = @"آزمایشی. متن خاکستری هم‌گام با حرف زدنت جلو می‌رود، ولی **خام است "
                    "و غلط دارد**؛ متنِ درست سر پایان می‌آید و سفید می‌شود. روی متنِ نهایی "
                    "هیچ اثری ندارد، ولی یک اتصال دوم به سرور باز می‌کند. **پیشنهاد ما "
                    "خاموش ماندن است**: خواندنِ حرفِ خودت وسط حرف زدن رشته‌ی کلام را پاره "
                    "می‌کند. فقط در حالت «جمع در پنل» (Command راست + P)";
    ...
    // «اختیاری» در خودِ عنوان، نه فقط در تولتیپ: تولتیپ را باید معطل ماند تا بیاید، و
    // کاربری که یک ردیفِ «کلید…» در منو می‌بیند تا همان لحظه فکر می‌کند اپ بی‌کلید کار
    ...
    // **یک سوال، یک ردیف.** قبلا دو ردیفِ تیک‌دار بود و هیچ‌کدام نمی‌گفت موضوع چیست: نه
    // معلوم بود این دو با هم یک انتخاب‌اند، نه اینکه اصلا دارند درباره‌ی چه چیزی تصمیم
    // می‌گیرند. حالا مثل «زبان» و «جای متن»، یک ردیف که مقدار فعلی را در خودش نوشته و
    // زیرمنویی که همان دو راه را با اسمِ کامل می‌گوید.
    //
    // و اسم‌ها از روی **کاری که با متن می‌شود** انتخاب شده‌اند، نه از روی مکانیزم داخلی:
    // یکی متن را همان‌جا می‌نویسد و به کلیپ‌بورد دست نمی‌زند، آن یکی متن را در کلیپ‌بورد
    // می‌گذارد و بعد می‌چسباند. کاربر با همین دو تفاوت طرف است، نه با «رویداد یونیکد».
    BOOL typing = ZSettings.shared.insertMode == ZInsertType;
    NSMenuItem *insItem = [self icon:[[NSMenuItem alloc]
                                      initWithTitle:typing ? @"روش نوشتن: درج مستقیم"
                                                           : @"روش نوشتن: ذخیره در کلیپ‌بورد"
                                             action:nil keyEquivalent:@""]
                               symbol:typing ? @"keyboard" : @"doc.on.clipboard"];
    insItem.toolTip = @"متن آماده چطور در برنامه‌ی مقصد نوشته شود: همان‌جا درج شود، یا اول "
                       "در کلیپ‌بورد بنشیند و بعد چسبانده شود. در Windows App همیشه راه "
                       "کلیپ‌بورد گرفته می‌شود و این انتخاب رویش اثری ندارد: درج مستقیم "
                       "آنجا فقط یک مشت a می‌نویسد.";
    NSMenu *insMenu = [NSMenu new];
    insMenu.autoenablesItems = NO;
    NSMenuItem *iType = [self item:insMenu title:@"درج مستقیم"
    ...
    iType.toolTip = @"زمزمه متن را حرف‌به‌حرف می‌نویسد، مثل کسی که پشت کیبورد نشسته. "
                     "به کلیپ‌بورد دست نمی‌زند، تقریبا همه‌جا کار می‌کند، ولی متن بلند "
                     "کند نوشته می‌شود.";
    NSMenuItem *iPaste = [self item:insMenu title:@"ذخیره در کلیپ‌بورد"
                             action:@selector(menuInsertMode:) key:@""];
    ...
    iPaste.toolTip = @"متن در کلیپ‌بورد می‌نشیند و زمزمه Command+V را می‌زند: آنی، حتی برای "
                      "متن بلند. برنامه‌ی مقصد باید چسباندن را قبول کند، و کلیپ‌بورد قبلی‌ات "
                      "جایش را به این متن می‌دهد.";
...
- (void)togglePreview {
    ...
    [_panel flash:on ? @"پیش‌نمایش روشن شد؛ خاکستری و خام، هم‌گام با حرفت"
