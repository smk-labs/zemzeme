...
# موجود ناسازگار است و هیچ import سوئیفتی (حتی Foundation) کامپایل نمی‌شود. اگر CLT
# را سالم خواستی:
#   sudo rm -rf /Library/Developer/CommandLineTools && xcode-select --install
#
# پورتِ سوئیفتِ نیمه‌کاره (app/swift-port/ [حذف‌شده]) برداشته شد: یک
# اسنپ‌شاتِ همان‌روزی بود که هیچ‌وقت به‌روز نشد، و تا امروز این کامنت خواننده را به
# درختی می‌فرستاد که وجود ندارد. زبانِ این اپ ObjC است، تا وقتی کسی عوضش نکند.
set -euo pipefail
cd "$(dirname "$0")"
