...
The optional AI passes (final pass, notes, prompt enhancement) need a free key from [Google AI Studio](https://aistudio.google.com/apikey). Once you have one, open the Zemzeme menu and click **"کلید Gemini…"** ("Gemini key…"), then paste it there. It's saved straight to your Mac's Keychain; nothing goes into the repo, a `.env` file, or any log. No terminal needed. (There's still a terminal path for scripting/headless use, see README.md.)
